...
# -*- coding: utf-8 -*-
"""
Particular function for loading and preparing/preprocessing the predictores/features for the analysis with the
statistical models.
"""
...
from ..Lists_and_Dictionaries.Features import feats_paper1
...
all_features = list(feats_paper1.keys())


# define the function
def load_feats_xlevel(reg_codes, ndlev=2, exposure=None, sel_feats=['s3', 'wspeed_mean'],
                      a_p="danger_level",
                      start_date="09-2016", end_date="09-2024",
                      h_low=400, h_hi=900,
                      agg_type="mean",
                      perc=90,
                      out_type="array",
    ...
    Parameters:
        ...
        a_p            String or List. The avalanche problem for which the danger level is chosen. Defaults to
                               ...
                               as NaN. Note that submitting "y" yields the same result as "danger_level".
    ...
    Note: Comparing this function with load_xlevel_preds, one may wonder why there is no parameter of NaN-handling. But
          this is indeed because the NaNs are already handled by that function when the predictive-feature files are
          generated, i.e., there are no NaNs in the files loaded here.
    ...
    # if a_p is "y" or ["y"] convert it to "danger_level"
    if ((a_p == "y") | (a_p == ["y"])):
        a_p = "danger_level"
    # end if
